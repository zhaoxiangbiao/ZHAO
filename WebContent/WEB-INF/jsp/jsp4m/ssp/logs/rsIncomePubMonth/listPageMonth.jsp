...
<div class="row">
        ...
        <div class="widget-buttons ">
        	已录入总收入：<span style="font-weight: bold;"><fmt:formatNumber groupingUsed="true" value="${sumData.gross_income }"></fmt:formatNumber> </span> 
            ...
            <ul class="pagination pagination-sm" style="margin-left: 15px;margin-top: 3px;">
                <li><a href="/ssp/logs/rsIncomePubMonth/listPageMonth?month=${p}" data-handler='DIV' target-div='#_main-page-body'>«</a></li>
                <li class="active"><a style="cursor: pointer;" id="_income_refresh_current_month" href="/ssp/logs/rsIncomePubMonth/listPageMonth?month=${m}" murl="/ssp/logs/adPubAdjustIncome/listPage?month=${m}"  data-handler='DIV' target-div='#_main-page-body'>${m}</a></li>
                <li><a href="/ssp/logs/rsIncomePubMonth/listPageMonth?month=${n }" data-handler='DIV' target-div='#_main-page-body'>»</a></li>
            </ul>
        </div>
        </div>
					<div class="data-div">
			...
			<div class="widget-header ">
				...
				<div class="widget-buttons">
							<form class="form-inline" role="form"   method="post" target="_BLANK" action="${urlPrev }exportExcel?excelName=${statDate }&sheetName=${statDate }&title=${statDate }"  id="form_${tableId}">
									<input type="hidden"  name="filter_EQS_stat_date" value="${statDate }" >
<!-- 									<input type="hidden"  name="fieldName" value="gross_income"  > -->
<!-- 									<input type="hidden"  name="compositorType" value="desc"  > -->
			...
			<div class="alert alert-warning fade in">
	                                         ...
	                                         <strong>注意1</strong> 月结表的收入只包括状态为【审核通过】的广告位收入.
	                                         <br/><i class="fa-fw fa fa-warning"></i> <strong>注意2</strong>【待支付】和【已支付】状态的月结算数据，在SSP端可见。
	                              </div>
 ...
 TableAjax.init('${tableId}',{
	 ...
	 uxEdit:{
		 edit:function (oTable, nRow) {
		     ...
		     jqTds[7].innerHTML = '<select class="form-control" name="input_country" data-bv-field="country"><option value="0" >无</option><option value="1" '+(aData.invoicetype==1?' selected ':' ')+'> 增值税发票</option><option value="1" '+(aData.invoicetype==2?' selected ':' ')+'> 普通发票</option></select>';
		     jqTds[8].innerHTML = '<input type="text"  class="form-control input-xs" style="text-align:1right;width:50px;font-size: 13px;padding:1px;" value="' + (aData['invoiceCount'] ?aData['invoiceCount']:'0' ) + '">';
		     jqTds[9].innerHTML = '<textarea type="text"  class="form-control input-xs" style="text-align:1right;width:150px;font-size: 13px;padding:1px;"  >' + (aData['invoicenumber'] ?aData['invoicenumber']:'' ) + '</textarea>';
		     jqTds[12].innerHTML = ' <a href="#" class="btn btn-warning btn-xs cancel"><i class="fa fa-times"></i> </a> '
         ...
         save:function (oTable, nRow) {
             ...
             var invoiceCount =  jqInputs[2].value;
             var invoicenumber =  jqInputs[3].value;
...
// 			{sTitle:'补贴',  mData: "subsidy",uxLongText:true,uxPrice:true,uxSum:true},				
			...
			{sTitle:'份数 ',  mData: "invoiceCount",sWidth:'50px',uxNumber:true,uxSum:true},				
			{sTitle:'发票编号',  mData: "invoicenumber",sWidth:'150px',uxTextArea:true},				

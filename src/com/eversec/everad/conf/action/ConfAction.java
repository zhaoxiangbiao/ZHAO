...
import  com.eversec.everad.conf.service.ConfService;

@Controller
@RequestMapping("/everad/conf/conf")
public class ConfAction extends BaseAction{
	...
	private String tableName = "null_conf";
	...
	public String listPage(HttpServletRequest request,Model model){
		...
		return SessionUtil.getViewPath("/everad/conf/conf/listPage");
	}
	...
	public Model listJSON(HttpServletRequest request,Model model){
		...
		PageData<Conf> pageData = new PageData<Conf>();
		//给pageData设置参数
		...
		pageData = confService.find(pageData);
	...
	public String add(Model model){
		return SessionUtil.getViewPath("/everad/conf/conf/input");
	}
	...
	public String editForm(Model model, String id) {
		Conf conf = confService.find(Long.parseLong(id));
	...
	public String viewForm(Model model, String id) {
		Conf conf = confService.find(Long.parseLong(id));
		model.addAttribute(conf);
		return SessionUtil.getViewPath("/everad/conf/conf/view");
	}
	...
	public String save(HttpServletRequest request, Model model, Conf conf){
		// 初始化
		returnCommand(model, request);
		if (conf.isNew()){
			confService.save(conf);
			//排序
			setSort4insert(this.sort,confService,this.tableName,this.sortField,conf.getId());
		} else{
	...
	public String delete(HttpServletRequest request, Model model, String ids){
		// 初始化
		returnCommand(model, request);
		delete(confService, "Conf", ids,logicDelete);
	...
	public String sort(HttpServletRequest request, Model model,Integer id, Integer sort,Integer afterSort,Integer beforeSort,Integer p){
		...
		confService.sort(tableName,id,sort,afterSort,beforeSort,p,sortField);
	...
	public String importView(HttpServletRequest request, Model model) {
		request.setAttribute("uri", "/everad/conf/conf/importExcel");
		request.setAttribute("navTabId", "everad/conf_conf_list");
	...
	public String importExcel(HttpServletRequest request, @RequestParam("upload") MultipartFile file, Model model) {
		this.importExcel(model, request, file, confService, new Conf());
		return SessionUtil.getViewPath("/commons/ajaxDone");
	}
	...
	public void exportExcel(HttpServletRequest request, HttpServletResponse response, String excelName, String sheetName, String title) {
		exportExcel(confService, request, response, excelName, sheetName, title);
	}

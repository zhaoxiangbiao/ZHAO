...
import  com.eversec.everad.conf.service.DomainService;

@Controller
@RequestMapping("/everad/conf/domain")
public class DomainAction extends BaseAction{
	...
	private String tableName = "null_domain";
	...
	public String listPage(HttpServletRequest request,Model model){
		...
		return SessionUtil.getViewPath("/everad/conf/domain/listPage");
	}
	...
	public Model listJSON(HttpServletRequest request,Model model){
		...
		PageData<Domain> pageData = new PageData<Domain>();
		//给pageData设置参数
		...
		pageData = domainService.find(pageData);
	...
	public String add(Model model){
		return SessionUtil.getViewPath("/everad/conf/domain/input");
	}
	...
	public String editForm(Model model, String id) {
		Domain domain = domainService.find(Long.parseLong(id));
	...
	public String viewForm(Model model, String id) {
		Domain domain = domainService.find(Long.parseLong(id));
		model.addAttribute(domain);
		return SessionUtil.getViewPath("/everad/conf/domain/view");
	}
	...
	public String save(HttpServletRequest request, Model model, Domain domain){
		...
		if (domain.isNew()){
			...
			//排序
			setSort4insert(this.sort,domainService,this.tableName,this.sortField,domain.getId());
		} else{
	...
	public String delete(HttpServletRequest request, Model model, String ids){
		// 初始化
		returnCommand(model, request);
		delete(domainService, "Domain", ids,logicDelete);
	...
	public String sort(HttpServletRequest request, Model model,Integer id, Integer sort,Integer afterSort,Integer beforeSort,Integer p){
		...
		domainService.sort(tableName,id,sort,afterSort,beforeSort,p,sortField);
	...
	public String importView(HttpServletRequest request, Model model) {
		request.setAttribute("uri", "/everad/conf/domain/importExcel");
		request.setAttribute("navTabId", "everad/conf_domain_list");
	...
	public String importExcel(HttpServletRequest request, @RequestParam("upload") MultipartFile file, Model model) {
		this.importExcel(model, request, file, domainService, new Domain());
		return SessionUtil.getViewPath("/commons/ajaxDone");
	}
	...
	public void exportExcel(HttpServletRequest request, HttpServletResponse response, String excelName, String sheetName, String title) {
		exportExcel(domainService, request, response, excelName, sheetName, title);
	}

...
import  com.eversec.everad.conf.service.LabelsService;

@Controller
@RequestMapping("/everad/conf/labels")
public class LabelsAction extends BaseAction{
	...
	private String tableName = "null_labels";
	...
	public String listPage(HttpServletRequest request,Model model){
		...
		return SessionUtil.getViewPath("/everad/conf/labels/listPage");
	}
	...
	public Model listJSON(HttpServletRequest request,Model model){
		...
		PageData<Labels> pageData = new PageData<Labels>();
		//给pageData设置参数
		...
		pageData = labelsService.find(pageData);
	...
	public String add(Model model){
		return SessionUtil.getViewPath("/everad/conf/labels/input");
	}
	...
	public String editForm(Model model, String id) {
		Labels labels = labelsService.find(Long.parseLong(id));
	...
	public String viewForm(Model model, String id) {
		Labels labels = labelsService.find(Long.parseLong(id));
		HashMap<String, String> phoneNumDetail =  labelsService.listPhoneNumDetail(labels);
		model.addAttribute(labels);
		model.addAttribute("phoneNumDetail",phoneNumDetail);
		return SessionUtil.getViewPath("/everad/conf/labels/view");
	}
	...
	public String save(HttpServletRequest request, Model model, Labels labels){
		// 初始化
		returnCommand(model, request);
		if (labels.isNew()){
			labelsService.save(labels);
			//排序
			setSort4insert(this.sort,labelsService,this.tableName,this.sortField,labels.getId());
		} else{
	...
	public String delete(HttpServletRequest request, Model model, String ids){
		// 初始化
		returnCommand(model, request);
		delete(labelsService, "Labels", ids,logicDelete);
	...
	public String sort(HttpServletRequest request, Model model,Integer id, Integer sort,Integer afterSort,Integer beforeSort,Integer p){
		...
		labelsService.sort(tableName,id,sort,afterSort,beforeSort,p,sortField);
	...
	public String importView(HttpServletRequest request, Model model) {
		request.setAttribute("uri", "/everad/conf/labels/importExcel");
		request.setAttribute("navTabId", "everad/conf_labels_list");
	...
	public String importExcel(HttpServletRequest request, @RequestParam("upload") MultipartFile file, Model model) {
		this.importExcel(model, request, file, labelsService, new Labels());
		return SessionUtil.getViewPath("/commons/ajaxDone");
	}
	...
	public void exportExcel(HttpServletRequest request, HttpServletResponse response, String excelName, String sheetName, String title) {
		exportExcel(labelsService, request, response, excelName, sheetName, title);
	}
